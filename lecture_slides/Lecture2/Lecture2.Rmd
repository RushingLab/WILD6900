...
In some cases, the probability of one event occuring is *independent* of whether or not the other event occurs$^1$  

In our example, the occupancy of the two species may be totally unrelated  

+ if they occur together, it happens by complete chance$^2$  
...
???
$^1$For example, the probability of a coin flip being heads is not dependent on whether or not the previous flip was heads. 

$^2$This maybe unlikely since even if they don't interact with each other, habitat preferences alone might lead to non-independence but we'll discuss that in a more detail shortly
...
In our example, maybe species $x$ and species $y$ **never** occur together$^1$  
...
$^1$Perhaps they are such fierce competitors that they will exclude each other from their territories  
...
As ecologists, there are a number of very common probability distributions that we encounter and use regularly$^1$: 
...
$^1$We are not going to go over the properties of each of these distributions in lecture. Instead, I will talk about specific distributions as they come up in examples. 
...
--
**Continuous** random variables can take on an infinite number of values on a specific interval$^1$  
...
--
**Discrete** random variables are those that take on distinct values, usually integers$^2$  
...
$^1$We usually encounter continuous variables in the form of regression coefficients (slope and intercepts), measurements (mass, lengths, etc.), and probabilities  

$^2$We usually encounter discrete variables in the form of counts (the number of individuals can only be postive integers, you can't have 4.234 individuals) or categories (alive vs. dead, present in location A vs. B vs. C)
...
--
Answering this question requires the use of probability functions, which we will denote $[z]^1$  

--
Probability functions differ between *continuous* and *discrete* distributions so we will discuss these separately  

???
$^1$Probability functions tell us $[z]=Pr(z)$  
...
- takes two arguments: the value we are interested in estimating the probability of $(z)^1$ and the expected value of our distribution $(\lambda)$
...
???
$^1$R will let us put in a vector of values so we can also do the following to estimate the probability of all values from 0 to 25: `dpois(x = seq(0, 25), lambda = 10)`  
...
Estimating probability density in R is the same as for discrete variables: `dnorm()`$^1$

???

$^1$Now you know why the function starts with `d`!
