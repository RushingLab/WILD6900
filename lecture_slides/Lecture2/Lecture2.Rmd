...
The *joint* probability $Pr(A,B)$ refers to the probability that two or more events occur and is usually denoted $^4$  
...
- Parent nodes are on the RHS of condition  
...
ggplot(norm_df, aes(x = z, y = pr_z)) + 
  ...
  geom_area(data=data.frame(z=seq(-2,-1,len=100),
                ...
                fill=WILD6900_colors$value[WILD6900_colors$name=="warning"], alpha = 0.3)
```
...
ggplot() +
  geom_path(data = norm_df, aes(z, pr_z), color = WILD6900_colors$value[WILD6900_colors$name=="primary"]) +
...
ggplot() +
  ...
  geom_vline(xintercept = c(0, 5, 10), linetype = "longdash", 
             color =  WILD6900_colors$value[WILD6900_colors$name %in% c("primary", "warning", "danger")])
