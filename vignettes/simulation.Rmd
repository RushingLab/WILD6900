...
```{r}
a <- c(beta0, beta1, beta2)
X <- cbind(1, sim_df$elevation.c, sim_df$rainfall.c)
```

```{r}
sim_df <- dplyr::mutate(sim_df, log.lambda = a %*% t(X), 
                                lambda = exp(log.lambda))

```

```{r}
ggplot(sim_df, aes(x = elevation, y = lambda)) + geom_point()
```
