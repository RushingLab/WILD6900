...
ggplot(beta_df, aes(x = phi, y = density, color = dist)) + geom_path() +
  ...
  scale_color_manual(values = WILD6900_colors$value[WILD6900_colors$name %in% c("primary", "success", "warning", "danger")])
```
...
Given our domain expertise, a reasonable choice for the mean and variance of the prior might be $\mu=0.9$ and $\sigma^2=0.03$. Plugging those values into equations 1 and 2 gives:

$$\alpha = \bigg(\frac{1-0.9}{0.03}- \frac{1}{0.9} \bigg)0.9^2 = 1.8$$ 
$$\beta = 3.6 \bigg(\frac{1}{0.9}-1\bigg) = 0.2$$
...
ggplot(beta_df, aes(x = phi, y = density)) + 
...
ggplot(beta_df, aes(x = phi, y = density, color = Approach)) + geom_path() +
  scale_x_continuous(expression(phi), limits = c(0.5, 1)) +
  scale_color_manual(values = WILD6900_colors$value[WILD6900_colors$name %in% c("primary", "success")]) 
...
ggplot() + 
  geom_histogram(data = sim_df, aes(x, stat(density)), alpha = 0.4,
                 color = "grey50",
                 fill =  WILD6900_colors$value[WILD6900_colors$name == "warning"], binwidth = 0.005) +
  geom_path(data = beta_df, aes(x = phi, y = density, color = Approach)) +
  scale_x_continuous(expression(phi), limits = c(0.7, 1)) +
  scale_color_manual(values = WILD6900_colors$value[WILD6900_colors$name %in% c("primary", "success", "warning")])
```

## Questions to consider

1) Informative vs vague: for informative priors: We want posteriors to reflect (and be affected by) prior knowledge.  against informative priors: why do the study if we claim to basically know the answer.

2) What defines 'sensitivity to the prior'. Should this be quantified by a change in the posterior mean, sd, some overlap metric, etc? Why use informative priors if the posterior is unaffected?

3) Process vs sampling noise: We included 14 studies reflecting a range of conditions and population trajectories. These 14 point estimates of survival reflect the process noise across populations, which is what we wanted to capture in our prior (option 1). The sampling uncertainty within a study was of less interest (option 2). 

4) Life history: Incorporating study-specific uncertainty generally increased prior weight for unreasonably low values of survival relative to option 1, which did not agree with expert opinion or life history characteristics for this species. 

5) Posterior distributions were relatively similar between options 1 and 2, if there was a change it generally reduced posterior mass for ecologically unreasonable (low) values. Other parameters were unaffected.
