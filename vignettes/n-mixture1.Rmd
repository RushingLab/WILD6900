...
With the data simulated, all you have to do next is package the data, set the MCMC settings, and run the model. Notice that we set `nAlpha` and `nBeta` based on the dimensions of the covariate arrays so that if we change the simulation, the data list will automatically update to the correct number of parameters. 
