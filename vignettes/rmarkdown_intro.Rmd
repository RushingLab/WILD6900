...
This is called the "YAML header" and it's where we can control a lot of the major formatting options for our documents. For example, to change the output to pdf, just switch `html_document` for `pdf_document` and then click the `Knit` button again
...
Inline code is useful to including (simple) `R` output directly into the text. Inline code can be added by inclosing `R` code between `\x60r` and `\x60`. For example, typing ` \x60r mean(c(3,7,4,7,9))\x60` will compute and print the mean of the given vector. This is, it will print `r mean(c(3,7,4,7,9))` instead of the code itself. This can be very useful for including summary statistics in reports. 

For example, if we have a vector indicating the number of individuals captured at each occassion during a mark-recapture study (e.g., `n <- c(155, 132, 147, 163)`) and we want to include the number of occasions in a report, instead of typing `4`, we can type `\x60r length(n)\x60`. Not only does this prevent typos, it is extemely useful if `length(n)` might change in the future. Instead of manually changing the number of occasions, we just re-render the document and the new number of occasions will be printed automatically. 

## Code chunks

For more complicated code, it is generally more useful to use **chunks** than inline code. Chunks start on a separate line with `\x60\x60\x60{r}` and end with a `\x60\x60\x60` on its own line (instead of doing this manually, you can click the `Insert` button at the top right of script window, then click `R`). In between these two lines, you can include as many lines of code as you want. For example,
