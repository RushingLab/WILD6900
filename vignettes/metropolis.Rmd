...
When generated the seed count model, we did so assuming a linear model that linked pollination visits to seed counts:

$$y_i = \alpha + \beta * x_i + \epsilon_i$$
$$\epsilon_i \sim Normal(0, \sigma^2)$$

This model includes 3 random variables that we want to estimate posterior distributions for: $\alpha$, $\beta$, and $\sigma^2$ (remember that the $y_i$ and $x_i$ are *observed* and therefore not modeled as *fixed* and *known* rather than random variables governed by probability distrubtions). So:

$$[\alpha, \beta, \sigma^2|y_i] \propto [y_i|\alpha, \beta, \sigma^2][\alpha][\beta][\sigma^2] \tag{1}$$

Sticking with the data generating model, we can define the likelihood of our model using a Normal distribution. So for given values of $\alpha$, $\beta$, and $\tau$, we estimate the likelihood as:
...
At each interation of the sampler, we will need to perform the same set of tasks, namely estimating the likelihood and joint distributions for each proposed and current parameter values. We could do this by copying and pasting the code necessary for these calculations at each point in the sampler code where we need them. However, any time you find yourself copying and pasting the same code more than once or twice, it's a good idea to consider wrapping that code in a function, which will do here. 
...
In the sampler, we loop over each parameter, going through the Metroplis steps for each and treating the other parameters as known (based on their current value in the chain). If this in not clear, walking through the `R` code will hopefully make it more concrete. 
...
Let's go through this line by line. In `step 1a`, we generate a new value of $\alpha$ from a normal distribution centered on the previous value. Remember that the tuning parameter we set earlier determine how big the jumps are between the current and proposed values. 
...
In `step 1c`, we estimate the joint probability of both the current and proposed values by addint the log likelihood to the log prior probability of each value (note that sum of the log values is the same as the product of the probabilities). 
...
## 3b: Calculate likelihood at candidate value
      candLlh <- calc_like(y = y, alpha = mcmc_df$alpha[i], beta = mcmc_df$beta[i], 
                          tau = cand, x = x)
